...
class SystemXyz2ChemdoodleJson(webapp2.RequestHandler):
    def post(self):
        self.response.headers.add_header("Access-Control-Allow-Origin", "*")
        xyz = self.request.get('xyz')
        s = system.read_xyz(xyz)
        self.response.write(s.write_chemdoodle_json('string'))


app = webapp2.WSGIApplication([
    ('/api', MainPage),
    ('/api/system/readlammps', SystemReadLammps),
    ('/api/system/readpubchemsmiles', SystemReadPubchemSmiles),
    ('/api/system/applyforcefield', SystemApplyForcefield),
    ('/api/system/writelammps', SystemWriteLammps),
    ('/api/system/writexyz', SystemWriteXyz),
    ('/api/system/lmps2xyz', SystemLmps2Xyz),
    ('/api/system/writechemdoodlejson', SystemWriteChemdoodleJson),
    ('/api/system/pubchemsmiles2chemdoodlejson', SystemPubchemSmiles2ChemdoodleJson),
    ('/api/system/xyz2chemdoodlejson', SystemXyz2ChemdoodleJson),
], debug=True)
