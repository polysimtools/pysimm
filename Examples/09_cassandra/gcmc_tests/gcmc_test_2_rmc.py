...
restart_ind = 4
mc_restart_dump = 'gcmc_test_2/MD' + str(restart_ind) + '_out.xyz'
...
gcmcPropsRead['Start_Type'] = OrderedDict([('start_type', 'read_config'),
                                           ('species', [1] + gas_mc.made_ins),
                                           ('file_name', mc_restart_dump)])
gcmc = cassandra.GCMC(gas_mc, fixed_sst, out_folder=out_folder, props_file=str(restart_ind + 1) + '.gcmc_props.inp',
                      **gcmcPropsRead)
css.add_gcmc(gcmc)
css.run()
...
while l < L_MAX:
    # >>> 2N: MD (LAMMPS) step:
    sim = lmps.Simulation(gcmc.tot_sst, print_to_screen=True, cutoff=14.0,
                          log=os.path.join(out_folder, str(l) + '.md.log'))
