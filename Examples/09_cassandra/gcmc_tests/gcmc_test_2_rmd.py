...
restart_ind = 3
md_restart_dump = 'gcmc_test_2/' + str(restart_ind) + '.before_md.lmps'
...
fixed_ref_sst = system.read_lammps(md_restart_dump)
...
gcmc = cassandra.GCMC(gas_mc, fixed_sst, out_folder=out_folder, props_file='some_props.inp', **gcmcPropsRead)
gcmc.upd_simulation()
...
while l < L_MAX:
    # >>> 2N: MD (LAMMPS) step:
    # gcmc.tot_sst.write_lammps(os.path.join(out_folder, str(l) + '.before_md.lmps'))
